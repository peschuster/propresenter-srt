...
if (!process.argv || process.argv.length < 7) {
    ...
    console.log('')
    console.log('Usage:')
    const argv0 = process.pkg ? process.argv0 : (process.argv0 + ' ' + ((process.argv && process.argv.length) > 1 ? process.argv[1] : 'index.js'))
    console.log(`  ${argv0} <propresenter-ip> <propresenter-port> <stagedisplay-password> [export-filename] [translations (0|1)]`)
    console.log('')
